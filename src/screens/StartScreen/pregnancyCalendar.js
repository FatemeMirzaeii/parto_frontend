...
const pregnancyCalendar = (props) => {
    ...
    function dayPress(day) {
        ...
        switch (type) {
            case 'zygosisDate':
                questionArray.push(
                    { periodDate: 0, childbirthDate: 0, zygosisDate: day.dateString })
                break;
            case 'childbirthDate':
                questionArray.push(
                    { periodDate: 0, childbirthDate: day.dateString, zygosisDate: 0 })
                break;
            case 'lastPeriodDate':
                questionArray.push(
                    { periodDate: day.dateString, childbirthDate: 0, zygosisDate: 0 })
                break;
        }
        props.navigation.navigate("StartQuestion5", { questionArray, forgetPragnancy: true })
    }
    switch (type) {
        case 'zygosisDate':
            return (
